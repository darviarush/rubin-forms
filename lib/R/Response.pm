package R::Response;
# содержит параметры, необходимые дл€ ответа
...
# сбрасывает объект в начальное состо€ние
sub reset {
	my ($self) = @_;
	my $app = $self->{app};
...
# перенаправление на другой url. ¬ ajax перенаправление происходит на сервере, а иначе - на клиенте
sub redirect {
	my ($self, $url, $text) = @_;
	$url = $self->{app}->request->referer("/") unless $url;
	$self->{status} = 303;	# 307 - перенаправл€ет и POST, а 303 - GET
...
# устанавливает ошибку - мен€ет body
sub error {
...
# добавл€ет ошибку дл€ элемента формы
sub addError {
...
sub ajax_redirect {
	...
	$self->{redirect} //= 0;
	die "√лубина редиректа больше 5. «ацикливание" if $self->{redirect}++ > 5;
