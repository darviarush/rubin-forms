...
sub sel (@) {
	my ($tab, $view, @args) = @_;
	join "", "SELECT ", FIELDS($view), " FROM ", SQL_WORD($tab), R::Utils::Connect::CONS::add(" ", \@args);
}

package R::Utils::Connect::JOIN;
sub new { my ($cls, $join, @args) = @_; my ($tab) = @args; my ($as); ($as, $tab) = R::Utils::Connect::TAB($tab); bless {tab => R::Utils::Connect::SQL_WORD($tab), as => R::Utils::Connect::SQL_WORD($as), args=>[@args], join=>$join}, $cls; }

package R::Utils::Connect::CONS;
...
sub compile {
	...
	my $val = $self->{val};
	if($op==1) {	# GROUP
		push @{$push->{group}}, R::Utils::Connect::FOR_TAB_FIELDS($val, $as);
	} elsif($op==2) { # HAVING
		push @{$push->{having}}, R::Utils::Connect::DO_WHERE($val, $as);
	} elsif($op==3) { # ORDER
		push @{$push->{order}}, R::Utils::Connect::FOR_TAB_FIELDS($val, $as);
	} elsif($op==4) { # LIMIT
...
sub add {
	...
	my $push;
	for my $arg (@$args) {
		if(ref $arg eq 'R::Utils::Connect::CONS') { $arg->compile($push) }
		else { push @{$push->{where}}, R::Utils::Connect::DO_WHERE($arg) }
	}
	R::Utils::Connect::CONS::join($push, $sep);
}
...
sub LEFT_JOIN (@) {	R::Utils::Connect::JOIN->new('LEFT', @_) }
sub INNER_JOIN (@) { R::Utils::Connect::JOIN->new('INNER', @_) }

# понятно
sub GROUP (@) { R::Utils::Connect::CONS->new(1, @_) }
sub HAVING (@) { R::Utils::Connect::CONS->new(2, @_) }
sub ORDER (@) { R::Utils::Connect::CONS->new(3, @_) }
sub LIMIT (@) { R::Utils::Connect::CONS->new(4, @_) }
...
sub sel_join (@) {
	...
	while(@st) {
		...
		my @real = ();
		for my $arg (@args) {
			if(ref $arg eq "R::Utils::Connect::JOIN") {
			...
			} elsif(ref $arg eq "R::Utils::Connect::CONS") {
				$arg->compile($push, $as);
			} else {
	...
	my $sql = join "", "SELECT ", join(", ", @view), "\nFROM ", SQL_WORD($table), ($table ne $as_table? (' as ', $as_table): ()), @join, R::Utils::Connect::CONS::join($push, "\n");
...
sub quick_rows {
	...
	my @join = grep { ref $_ eq 'R::Utils::Connect::JOIN' } @args;
	@args = grep { ref $_ ne 'R::Utils::Connect::JOIN' } @args;
